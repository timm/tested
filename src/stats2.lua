...
local function RX(t,s) 
  table.sort(t)
  return {name=s or "", rank=0, n=#t, show="", has=t} end

local function mid(t,     n)
...
local function merge(rx1,rx2,    rx3) 
  rx3 = RX({}, rx1.name)
...
local function scottKnot(rxs,      all,cohen)
...
local function tiles(rxs) --> ss; makes on string per treatment showing rank, distribution, and values
...
return {cliffsDelta=cliffsDelta, bootstrap=bootstrap}
