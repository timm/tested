...
- ALso, to quickly withdraw a number `x` from a Gaussian (using the same code as 101.lua) use
  the following (but this gets unstable for `n` under 10 and crashes for `n&lt;2`):
    ...
    self.mu -= d / self.n
    self.m2 -= d * (x - self.mu)
    self.sd  = (self.m2/(self.n-1))^.5 -- crashes when `m2` goes negative or `n` <2

_Personnel view:_ 
Not all things are normal Gaussians. 
<img src="/etc/img/weibull.png" align=right width=400 > If you want to get fancy, you can use Weibull functions
to make a variety of shapes (just by adjusting $\lambda,k$):

<p align=center><img src="/etc/img/weibull.png" wdith=300 ></p>
...
To go fully parametric, use reservoir sampling (below). Then to sample, grab three numbers $a,b,c$ and use $x=a+f(b-c)$ for some small $f$ (say $f=0.1$).
